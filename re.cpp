...
std::bitset<64> input_bitset64_live() {
    while (true) {
        std::string bits = get_live_bits(64);
        if (bits.empty()) {
        ...
        if (bits.size() < 64) {
            ...
            if (choice == 1) {
                ...
                while (bits.size() < 64) {
                    ...
                    std::string extra = get_live_bits(64 - bits.size());
                    bits += extra;
...
int main() {
    ...
    std::bitset<64> x = input_bitset64_live();
    ...
    do {
        std::cout << "Введите желаемый вес w (0 <= w <= 64): ";
        std::cin >> w;
    } while (w < 0 || w > 64);
